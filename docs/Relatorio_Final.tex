...
Observe que o ganho estático da planta é baixo devido aos pólos em alta frequência. Isso exigirá um ganho elevado do controlador para atender aos requisitos de erro.
...
\]
Aplicando o critério de Routh-Hurwitz, determinamos o ganho crítico ($K_{crit}$) a partir do qual o sistema se torna instável. Devido ao ganho reduzido da planta ($1.2$), o $K_{crit}$ será significativamente maior do que no modelo anterior.
...
Com $K_p = 77.000$, a simulação (Figura \ref{fig:step_p}) apresenta a resposta ao degrau. Comparado ao caso anterior ($K_p=138$ com planta antiga), agora temos:
\begin{itemize}
    \item Sistema Estável.
    \item Erro de regime elevado (necessitando Lag).
\end{itemize}
\subsubsection{Análise de Desempenho (Dierson - $K_p=77.000$)}
Utilizando o ganho $K_p = 77.000$ sugerido pelo estudo de Dierson para a planta sem $K_{sys}$:
\begin{itemize}
    \item \textbf{Estabilidade:} O sistema é \textbf{estável} (Margem de Fase positiva), ao contrário do que se esperaria se tivéssemos o ganho intrínseco. A margem de estabilidade é ampla ($K_{crit} \approx 12 \times 10^6$).
    \item \textbf{Erro de Regime:} O ganho de velocidade é $K_v = 77.000 \times \frac{1.2}{12540} \approx 7.3$.
    \[
    e_{rampa} = \frac{1}{K_v} \approx \frac{1}{7.3} \approx 13.7\%
    \]
    O erro é muito superior ao requisito de $1\%$. Portanto, o controlador P puro não atende à especificação de erro, exigindo um compensador Lag.
\end{itemize}
\begin{itemize}
...
\[
K_v = 77000 \times \frac{1.2}{12540} = 77000 \times 9.57 \times 10^{-5} \approx 7.37 \text{ s}^{-1}
\]
...
\begin{enumerate}
    ...
    \item \textbf{Novo Kv:} $K_v \approx 7.27 \times 10 \approx 72.7$.
    \item \textbf{Erro estimado:} $\approx 1.3\%$. (Simulação aponta valores próximos a 1\%).
    \item \textbf{Estabilidade:} O sistema mantém a estabilidade do Proporcional, pois o atraso de fase introduzido pelo dipolo ocorre em frequências muito baixas, não afetando a margem de fase na frequência de cruzamento ($\approx 250$ rad/s).
\end{enumerate}

\textbf{Conclusão:} A solução de Dierson ($K_p=76k$) é robusta e fisicamente coerente com a planta modelada.

\subsection{Novo Cálculo de Erro}
Calculamos o novo $K_v$ em malha aberta:
\[
K_v^{new} = \lim_{s\to 0} s \cdot C_{lag}(s) G(s) = \lim_{s\to 0} s \cdot \left( 76000 \frac{s+0.1}{s+0.01} \right) \frac{1.2}{s(s+13.2)(s+950)}
\]
Calculando o valor numérico:
\[
K_v^{new} = 76000 \cdot \left( \frac{0.1}{0.01} \right) \cdot \frac{1.2}{12540} = 76000 \cdot 10 \cdot 9.57 \times 10^{-5} \approx 72.7
\]
O novo erro de rampa estimado é:
\[
e_{ss} = \frac{1}{72.7} \approx 1.37\%
\]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../assets/report_images/robustness_PID.png}
